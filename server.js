...
function isVipLeague(name = "") {
  ...
  return VIP_LEAGUES.some(l => lower.includes(l.toLowerCase()));
}
...
function confidence(minute, homeGoals, awayGoals, totalDiff, side) {
  let c = 5;
...
async function fetchLiveMatches() {
  try {
    ...
    console.log(
      `API TEST: ${matches.length} canlı maç | İncelenecek: ${filteredMatches.length}`
    );
...
`);
    ...
    console.log(
      `ÖZET → StatsBakıldı:${checkedStats} | StatsYok:${skippedNoStats} | MomentumZayıf:${weakMomentum} | MarketYok:${noMarket} | GüvenDüşük:${lowConfidence} | ValueDüşük:${lowValue}`
    );
  } catch (err) {
...
console.log("💰 VIP PARA BOTU LIMIT DOSTU aktif...");

setTimeout(() => {
  sendTelegram("✅ VIP PARA BOTU LIMIT DOSTU aktif.")
